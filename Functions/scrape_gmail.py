...
pinecone.init(      
	api_key='ee5b8ecd-7b21-46dd-80c5-c46887946993',      
	environment='asia-southeast1-gcp-free'      
)      

# Conexão com o índice
index = pinecone.Index(index_name="bregabregosoex")

# Enviar dados para o índice
for email_text in all_email_text:
    index.upsert(ids=[str(email_text)], vectors=[email_text])

# Texto de exemplo para consulta
query_text = "A.I., AI, A.I, Newsletter"

# Realizar consulta
query_results = index.query(queries=[query_text], top_k=5)

# Resultados da consulta
for result in query_results[0].results:
    similar_email_text = result.id
    similarity_score = result.score
    print(f"Texto semelhante: {similar_email_text}, Pontuação de Similaridade: {similarity_score}")
